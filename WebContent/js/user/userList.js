			...
			.confirm({
				...
				buttons : {
					确认 : {
						...
						type : "blue",
						action : function() {
							if ($("input[name='user_username']").val() != ""
...
function add_computerAjax() {
	var formData = new FormData();
	formData.append("user.user_username", $("input[name='user_username']")
			.val());
	formData.append("user.user_name", $("input[name='user_name']").val());
	formData.append("user.user_password", $("input[name='user_password']")
			.val());
			...
			确认 : {
				...
				action : function() {
					...
					$('.cloudList_tabel tbody').find(
							'input[name="delete_check"]').each(function(i) {
						if ($(this).is(':checked')) {
							formData.append('user_id', $(this).attr('id'));
							console.log("要删除的id" + $(this).attr('id'));
							HaveDate = true;
							index++;
						}
					});
...
function goPage() {
	...
	if ($("#goInput").val() <= user_paginationQuery.totalPage
			&& $("#goInput").val() >= 1) {
